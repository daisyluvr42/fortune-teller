...
3. **能量解释**：不要只说"喜火忌水"。用**"能量电池"**比喻解释喜忌为什么有效。

请严格按以下结构输出（使用 Markdown）：

## 1. 🔋 你的能量诊断书（强弱分析）
* **元神状态**：用自然意象描述日主。（例如："你是冬天里的一把篝火，明亮但需要木材续燃。"）
* **核心结论**：明确判定"身强"或"身弱"。

## 2. ✨ 你的喜用/忌用清单（能量维他命）
* **幸运元素**：明确指出最有利五行（金/木/水/火/土）。
* **忌神提醒**：指出需要警惕的五行，并解释过量的具体负面感受。
* **底层逻辑**：用大白话解释为什么要用/避开。

## 3. 🎨 生活开运方案（需联网检索）
* **幸运色与穿搭**：搜索 **{this_year}-{next_year} 流行色**，推荐符合喜用五行的具体色号。
* **能量补给地（方位/旅行）**：结合喜用方位推荐 1-2 个具体城市/国家。
* **开运数字**：推荐 1-2 个手机尾数或密码组合。
* **黄金行动时间**：指出一天中高效时段与幸运季节。
* **微习惯处方**：给一个可执行的小习惯建议。
...
⚠️ **关键约束**：
1. **边界控制**：严禁给出具体职业选择、情感对象分析或开运物品建议（这些由其他模块负责）。
2. **聚焦趋势**：重点描述能量“形态”（扩张/收缩/动荡/停滞）与“心理体感”（焦虑/自信/压抑/通透）。
3. **逻辑透明**：结论必须对应五行/十神的生克制化，但需转化为“环境描述”与“体感描述”。
